...
First, when the proportion of bidirectional edges is fixed to 0.5, we compare the performance of our method with DAG-GNN. The results are shown in Figure \ref*{SHD_FDR_prop50}. Our method outperforms DAG-GNN in terms of both SHD and FDR. Also, at the node size of 20, our method has shown stable training, which results a huge difference in SHD and FDR. The example of estimated graphs of each method is shown in Figure \ref*{example_graphs}. The estimated graph from DAG-GNN(third column) has much more edges than the true graph(first column), while our method(second column) has much similar number of edges with the true graph.\\
...
Next, we compare the performance of two methods with different proportions of bidirectional edges. The results are shown in Figure \ref*{SHD_FDR_node10_20}. When the node size is 10, DAG-GNN outperforms our method in terms of SHD. However, when the node size is 20, our method outperforms DAG-GNN in terms of both SHD and FDR.\\
